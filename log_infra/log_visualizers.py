...
class LogVisualizer(abc.ABC):
    ...
    @abc.abstractmethod
    def create_viz_img(self, io_dict: Dict[str, Dict[str, Tensor]], *args, **kwargs) -> Image.Image:
...
class OrigVsReconLogVisualizer(LogVisualizer):
    ...
    def __init__(self,
        ...
        """
        path_io are the path and io in the io_dict (e.g., 'network.g_s', 'output')
    ...
    def create_viz_img(self, io_dict: Dict[str, Dict[str, Tensor]], *args, **kwargs) -> Image.Image:
        ...
        return concat_images_h(
            self.to_img(
                io_dict[self.from_model_orig][self.orig_module_path][self.orig_module_io][0].detach().squeeze()),
            self.to_img(
                io_dict[self.from_model_recon][self.recon_module_path][self.recon_module_io][0].detach().squeeze()))
...
class AttentionMapsVisualizer(LogVisualizer):
    ...
    def _max_of_absolute_p(features: Tensor, p: int, *args, **kwargs) -> Tensor:
        """
            b x C x H x W  --> b x 1 x H x W
        """
        return torch.pow(torch.max(torch.abs(features), dim=1, keepdim=True)[0], exponent=p)
    ...
    def create_viz_img(self, io_dict: Dict[str, Dict[str, Tensor]], *args, **kwargs) -> Image.Image:
        ...
        for feature_module_path, feature_module_io in self.path_io_features:
            ...
            res = concat_images_h(Image.fromarray(masked_img), res)
        return res
...
class LatentLogVisualizer(LogVisualizer):
    ...
    def __init__(self,
        ...
        self.from_model_comparison, self.module_path_comparison, self.module_io_comparison = paths_io_comparison
        self.resolution = resolution

    def create_viz_img(self, io_dict: Dict[str, Any], *args, **kwargs) -> Image.Image:
        latent = io_dict[self.from_model][self.module_path][self.module_io][0].squeeze().detach().cpu().numpy()
        img = self.to_img(F.upsample(latent, size=self.resolution))
        if self.from_model_comparison:
            latent_comparison = \
                io_dict[self.from_model_comparison][self.module_path_comparison][self.module_io_comparison][
                    0].squeeze().detach().cpu().numpy()
            latent_comparison = self.to_img(F.upsample(latent_comparison, size=self.resolution))
...
def get_log_visualizers(visualizers_config: List[Mapping[str, Any]]) -> List[LogVisualizer]:
    visualizers = list()
    missing = []
