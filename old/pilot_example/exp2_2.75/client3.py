...
classes_to_keep_train = [6, 7, 8]
train_indices = [i for i in range(len(train_labels)) if train_labels[i][0] in classes_to_keep_train]

classes_to_keep_test = [0, 1, 2, 3, 4, 5, 6, 7, 8]
test_indices = [i for i in range(len(test_labels)) if test_labels[i][0] in classes_to_keep_test]
...
while True:
    try:
        # Create a Flower client and start the client loop
        fl.client.start_numpy_client(server_address="127.0.0.1:5050", client=CifarClient())
        break
    except:
        print("Received error. Restarting client...")
