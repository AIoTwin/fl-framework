...
class CustomFedAvg(fl.server.strategy.FedAvg):
    def aggregate_fit(
        ...
        state_dict = ndarray_to_weight_dict(
            model.state_dict().keys(), parameters_aggregated
        )
...
class EdgeServerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        print(
            "ClientThread - Sending parameters to cloud aggregation: "
            + str(current_edge_round)
        )
...
class clientThread(threading.Thread):
    ...
    def run(self):
        fl.client.start_numpy_client(
            server_address="127.0.0.1:5000", client=EdgeServerClient()
        )
