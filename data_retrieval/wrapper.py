...
from misc.util import check_if_exists
...
class CacheableDataset(BaseDatasetWrapper):
    ...
    def __getitem__(self, index):
        ...
        if check_if_exists(cache_file_path):
...
class LabelelessDataset(BaseDatasetWrapper):
    def __init__(self, org_dataset):
        super().__init__(org_dataset)

    def __getitem__(self, index):
        sample = self.org_dataset.__getitem__(index)
        return sample, dict()
...
class ContrastiveDataset(BaseDatasetWrapper):
    def __init__(self, org_dataset, num_negative_samples, mode, ratio):
        ...
        self.cls_positives = [[] for _ in range(num_classes)]
        for i in range(num_samples):
            self.cls_positives[labels[i]].append(i)

        self.cls_negatives = [[] for _ in range(num_classes)]
